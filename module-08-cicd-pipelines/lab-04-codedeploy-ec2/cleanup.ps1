...
$StackName = "dva-lab-08-04-codedeploy-ec2"
$Bucket = aws cloudformation describe-stacks --stack-name $StackName --query "Stacks[0].Outputs[?OutputKey=='ArtifactsBucket'].OutputValue" --output text 2>$null
if ($Bucket -and $Bucket -ne 'None') { aws s3 rm "s3://$Bucket" --recursive 2>$null }

Write-Host "Deleting stack: $StackName (EC2 termination takes a few min)"
